...
describe("Records Service", () => {
    let createRecordService;
    let getRecordsService;
    let updateRecordService;
    let deleteRecordService;
    beforeAll(async () => {
    ...
    describe("createRecordService", () => {
        it("throws if createdById is missing", async () => {
            await expect(createRecordService({
        ...
        it("creates record successfully", async () => {
            ...
            expect(mockPrisma.financialRecord.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    describe("getRecordsService", () => {
        it("handles pagination ranges correctly", async () => {
            ...
            expect(mockPrisma.financialRecord.findMany).toHaveBeenCalledWith(expect.objectContaining({
                skip: 0,
                take: 50,
            }));
        });
        it("throws on invalid dates", async () => {
            await expect(getRecordsService({ userId: "user-1", startDate: "invalid-date" })).rejects.toThrow("Invalid startDate format");
        });
    });
    describe("updateRecordService", () => {
        it("throws if record is not found", async () => {
            ...
            await expect(updateRecordService({ id: "record-1", userId: "user-1", amount: 200 })).rejects.toThrow("RECORD_NOT_FOUND");
        });
        it("updates record successfully", async () => {
            ...
            expect(mockPrisma.financialRecord.update).toHaveBeenCalledWith(expect.objectContaining({
                where: { id: "record-1" },
            }));
        });
    });
...
//# sourceMappingURL=records.service.test.js.map